...
const MONITOR_SETTINGS: MonitorSettings[] = [
  {
    ...
    settings: [
      ...
      { label: 'Smart HDR', value: 'OFF', why: 'HDR processing can add latency on some panels; verify with the monitor\'s own mode and a trusted latency test' },
      { label: 'AMD FreeSync / NVIDIA G-Sync Compatible', value: 'Test OFF first', why: 'Latency-first baseline when FPS stays above refresh. If frametime spikes or tearing are visible, compare VRR on with the panel and frame cap; there is no universal pro setting.' },
  ...
  },
  {
    ...
    settings: [
      ...
      { label: 'NVIDIA G-Sync', value: 'A/B test OFF vs ON', why: 'Use OFF as the latency-first baseline when the rig can hold refresh. If tearing or frametime variance matters more, test VRR with the same cap and Reflex setting; panel behavior and user sensitivity decide the winner.' },
      { label: 'Preset Mode', value: 'FPS', why: 'Locks color + gamma for competitive contrast' },
      { label: 'Smart HDR', value: 'OFF', why: 'HDR processing can add latency on some panels; verify the panel mode with a trusted latency test' },
  ...
  },
  {
    ...
    settings: [
      ...
      { label: 'G-Sync / FreeSync', value: 'A/B test per panel', why: 'At 240+ Hz, start with VRR off if FPS is stable above refresh, then compare against VRR on with identical game settings. Keep the mode that wins your frametime and tear-visibility check.' },
    ],
