...
- Cross-project: MaxxTopia commits `5fc5b5a` (earlier Sonic/Fastboi homepage attempt), `29dc11e` (source-wall fixed-gain cleanup), `64f6281` (homepage restore + short v0.4.0 teaser), and `18033d2` (single suite-style v0.4.1 update) are pushed; existing dirty/untracked MaxxTopia work was preserved. The v0.4.1 repository-dispatch sync and both Cloudflare deploys succeeded.
- Checks passed: `npm run audit:catalog` (100 tweaks, 0 errors / 0 warnings); `npx tsc --noEmit`; `cargo test --manifest-path src-tauri/Cargo.toml` (84 passed); `npm run build` after the final placement and changelog fixes; `git diff --check`; local browser inspection of app theme gating and live MaxxTopia routes.
